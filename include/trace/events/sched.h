...
TRACE_EVENT(sched_switch,
	...
	TP_printk("prev_comm=%s prev_pid=%d prev_prio=%d prev_state=%s%s ==> next_comm=%s next_pid=%d next_prio=%d prev->cgrp=%llu next->cgrp=%llu prev->st=%llu next->st=%llu",
		...
		, (unsigned long long)__entry->prev_cgrp_id
		, (unsigned long long)__entry->next_cgrp_id
		, (unsigned long long)__entry->prev_st_cgrp_id
		, (unsigned long long)__entry->next_st_cgrp_id
	)
